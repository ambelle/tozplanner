...
const STORAGE_KEY = 'tozAutoPlanner_v10_command_center';
...
const defaultPlayers = [
  ["Thocky","Aran",290,"Belle",1,7,true],["Tock","Night Walker",263,"Belle",1,7,false],["Tocki","Bowmaster",254,"Belle",1,7,false],["Tork","Blaze Wizard",261,"Belle",1,7,false],["Tokk","Buccaneer",208,"Belle",1,7,false],["Hell","Fire/Poison Mage",276,"Belle",1,7,true],["Tocky","Bishop",265,"Belle",1,7,false],
  ["Gillian","Dual Blade",284,"Gillian",1,7,true],["ggil","Bowmaster",265,"Gillian",1,7,false],["ggill","Buccaneer",247,"Gillian",1,7,false],["Leaw","Aran",265,"Gillian",1,7,false],["Gill","Fire/Poison Mage",276,"Gillian",1,7,true],["Kaiko","Bishop",267,"Gillian",1,7,false],["Arun","Dawn Warrior",243,"Gillian",1,7,false]
];
...
function addPlayer(ign, cls, level=250, owner="Public", from=1, to=7, mw40=false, silent=false, maxRuns=null){
  owner = normalizeOwner(owner);
  players.push({id:id(),ign,cls,level:+level||0,owner,avail:avail(+from,+to),mw40:!!mw40,maxRuns:maxRuns?+maxRuns:null});
...
function evaluate(r){
  ...
  if(!hasDispel){checks.push(["Missing Dispel","bad"]);score-=50}else checks.push(["Dispel OK",""]);
  if(fma===0){checks.push(["No FMA","bad"]);score-=40}else if(fma===1){checks.push(["Only 1 FMA","warn"]);score-=8}else checks.push(["2+ FMA",""]);
  const seOnly=ps.filter(p=>info(p).seOnly);
  if(seOnly.length && !hasSE){checks.push(["HARD: NL/NW/Evan need SE","bad"]);score-=60}else if(seOnly.length) checks.push(["SE-only covered",""]);
  const critNeed=ps.filter(p=>needs(p,"SE/TH"));
  if(critNeed.length && !(hasSE||hasTH)){checks.push(["HARD: Missing SE/TH","bad"]);score-=60}else if(critNeed.length) checks.push(["SE/TH covered",""]);
  ...
  const mwCount = ps.filter(p=>p.mw40).length;
  if(mwCount>1){checks.push([`${mwCount} MW40 chars here`,"warn"]);score-=3;}
  else if(mwCount===1){checks.push(["MW40 present",""]);}
...
function evalWith(ps){
  ...
  if(!hasDispel) score-=50; if(fma===0) score-=40; else if(fma===1) score-=8;
  if(ps.some(p=>info(p).seOnly)&&!hasSE) score-=60;
  if(ps.some(p=>needs(p,"SE/TH"))&&!(hasSE||hasTH)) score-=60;
  if(ps.some(p=>needs(p,"SI"))&&!hasSI) score-=12;
  if(owners.some((o,i)=>owners.indexOf(o)!==i)) score-=50;
  if(ps.filter(p=>p.mw40).length>1) score-=3;
  score-=Math.max(0,6-ps.length)*4;
  return {score:Math.max(0,score)};
}

function hasHardIssue(r){
  return evaluate(r).checks.some(c=>c[1]==='bad');
}

function canReplaceWith(candidateId,r,slot){
  const cand=getP(candidateId); if(!cand || !isAvailable(cand,r) || lockedSlots[r][slot]) return false;
  const old=plan[r][slot];
  if(old===candidateId) return true;
  // Temporarily remove the target slot so same-owner swaps are allowed.
  plan[r][slot]=null;
  const ok = canPlace(candidateId,r,slot);
  plan[r][slot]=old;
  return ok;
}
function bestReplacementFor(r, predicate){
  let best=null;
  for(let s=0;s<6;s++){
    if(lockedSlots[r][s]) continue;
    const old=plan[r][s];
    const base = roundPlayers(r).filter(p=>p.id!==old);
    for(const cand of players.filter(predicate)){
      if(!canReplaceWith(cand.id,r,s)) continue;
      const trial=[...base,cand];
      const score=evalWith(trial).score + candidateScore(cand,r,base);
      if(!best || score>best.score) best={slot:s,cand,score};
    }
  }
  return best;
}
function applyHardFix(r, predicate){
  const rep=bestReplacementFor(r,predicate);
  if(rep){ plan[r][rep.slot]=rep.cand.id; return true; }
  return false;
}
function isCritProvider(p){return provides(p,"SE") || provides(p,"TH");}
function weakestReplaceSlot(r, wantFn){
  let best=null;
  for(let s=0;s<6;s++){
    if(lockedSlots[r][s]) continue;
    const p=getP(plan[r][s]);
    let penalty = 0;
    if(!p) penalty = -100;
    else {
      if(CORE_OWNERS.has(p.owner)) penalty += 40;
      if(provides(p,"Dispel")) penalty += 35;
      if(provides(p,"SE")) penalty += 35;
      if(provides(p,"TH")) penalty += 25;
      if(info(p).fma) penalty += 15;
      if(info(p).seOnly || needs(p,"SE/TH") || needs(p,"SI")) penalty += 5;
    }
    if(best===null || penalty < best.penalty) best={slot:s,penalty};
  }
  return best ? best.slot : -1;
}
function forceFixRound(r){
  // HARD RULE 1: every round must have Dispel where possible.
  if(!roundHas(r,"Dispel")) applyHardFix(r, p=>provides(p,"Dispel"));

  // HARD RULE 2: every round must have at least 1 FMA where possible. 2 FMA is still preferred, but 1 is the minimum.
  if(fmaCount(r)===0) applyHardFix(r, p=>info(p).fma);

  // HARD RULE 3: NL / NW / Evan require actual SE, TH does not count.
  if(roundPlayers(r).some(p=>info(p).seOnly) && !roundHas(r,"SE")) applyHardFix(r, p=>provides(p,"SE"));

  // HARD RULE 4: any class tagged Needs SE/TH must have either SE or TH in the same party.
  if(roundPlayers(r).some(p=>needs(p,"SE/TH")) && !(roundHas(r,"SE")||roundHas(r,"TH"))) applyHardFix(r, p=>isCritProvider(p));

  // HARD RULE 5: after adding SE-only classes, re-check SE again in case a swap broke it.
  if(roundPlayers(r).some(p=>info(p).seOnly) && !roundHas(r,"SE")) applyHardFix(r, p=>provides(p,"SE"));
}
...
function candidateScore(p,r,current){
  ...
  let s = after-before;
  const currentNeedsCrit = current.some(x=>needs(x,"SE/TH"));
  const currentNeedsSEOnly = current.some(x=>info(x).seOnly);
  const currentHasSE = current.some(x=>provides(x,"SE"));
  const currentHasTH = current.some(x=>provides(x,"TH"));
  if(currentNeedsSEOnly && !currentHasSE && provides(p,"SE")) s += 120;
  if(currentNeedsSEOnly && !currentHasSE && !provides(p,"SE")) s -= 70;
  if(currentNeedsCrit && !(currentHasSE||currentHasTH) && isCritProvider(p)) s += 100;
  if(currentNeedsCrit && !(currentHasSE||currentHasTH) && !isCritProvider(p)) s -= 55;
  if(needs(p,"SE/TH") && !(currentHasSE||currentHasTH) && !isCritProvider(p)) s -= 45;
  if(info(p).seOnly && !currentHasSE && !provides(p,"SE")) s -= 65;
  ...
  if(p.mw40 && current.some(x=>x.mw40)) s-=22; // soft preference: split MW40 across different rounds when possible
  if(p.mw40 && !current.some(x=>x.mw40)) s+=4;
...
function autoPlan(onlyEmpty=false){
  ...
  for(let pass=0; pass<4; pass++) for(let r=0;r<7;r++) forceFixRound(r);
  for(let r=0;r<7;r++) improveRound(r);
  for(let pass=0; pass<3; pass++) for(let r=0;r<7;r++) forceFixRound(r);
  save();render();
}
...
function card(p){
  ...
  const jobClass = 'job-' + (info(p).short || p.cls || 'other').toLowerCase().replace(/[^a-z0-9]+/g,'-');
  div.classList.add(jobClass);
  if(p.mw40) div.classList.add('has-mw40');
  div.querySelector('.p-ign').textContent=p.ign; div.querySelector('.p-class').textContent=`${info(p).short} · Lv ${p.level}`;
  div.querySelector('.p-tags').innerHTML=[...(p.mw40?[`<span class="tag mw">MW40</span>`]:[]),...info(p).provides.map(t=>`<span class="tag key">${t}</span>`),...info(p).needs.map(t=>`<span class="tag need">Needs ${t}</span>`)].join('');
...
function load(){const raw=localStorage.getItem(STORAGE_KEY); if(raw){try{const d=JSON.parse(raw); players=(d.players||[]).map(p=>({...p,mw40:!!p.mw40})); plan=d.plan||plan; lockedSlots=d.lockedSlots||lockedSlots;}catch{}} if(!players.length) loadDefaults();}
...
function addParsedSignups(){
  ...
  for(const item of ok){
    ...
    item.classes.forEach(cls=>{
      ...
      addPlayer(ign, cls, 250, item.owner, item.from, item.to, false, true, item.maxRuns);
    });
  }
  save(); render();
...
document.getElementById('addPlayerBtn').onclick=()=>{ const ign=ignInput.value.trim()||`Player${players.length+1}`; const f=Math.min(+fromRound.value,+toRound.value), t=Math.max(+fromRound.value,+toRound.value); addPlayer(ign,classInput.value,levelInput.value,ownerInput.value.trim()||ign,f,t,false); ignInput.value=''; ownerInput.value=''; };
...
/* V10 Command Center layout overrides */
let activeRound = window.activeRound || 0;
...
function renderSummary(){
  const scores=[0,1,2,3,4,5,6].map(r=>evaluate(r).score);
  const critical=[0,1,2,3,4,5,6].filter(r=>evaluate(r).checks.some(c=>c[1]==='bad')).length;
  const filled=plan.flat().filter(Boolean).length;
  const coreLeft=[...CORE_OWNERS].map(o=>`${o}: ${players.filter(p=>p.owner===o && !inAnyPlan(p.id)).length}`).join(' · ');
  const avg=Math.round(scores.reduce((a,b)=>a+b,0)/7);
  const el=document.getElementById('summary');
  if(!el) return;
  el.innerHTML=`
    <div class="metric"><b>${players.length}</b><span>Total chars</span></div>
    <div class="metric"><b>${filled}/42</b><span>Filled slots</span></div>
    <div class="metric"><b>${avg}</b><span>Avg score</span></div>
    <div class="metric"><b>${critical}</b><span>Critical rounds</span></div>
    <div class="metric wide"><b>${coreLeft}</b><span>Core unused</span></div>`;
}

function renderRoundTabs(){
  const root=document.getElementById('roundTabs');
  if(!root) return;
  root.innerHTML='';
  for(let r=0;r<7;r++){
    const ev=evaluate(r);
    const bad=ev.checks.some(c=>c[1]==='bad');
    const filled=roundPlayers(r).length;
    const btn=document.createElement('button');
    btn.className='round-tab'+(r===activeRound?' active':'')+(bad?' bad':'');
    btn.innerHTML=`<div class="rt-title">Round ${r+1}</div><div class="rt-meta"><span>${ev.score}/100</span><span>${filled}/6</span></div><div class="mini-bar"><div class="mini-fill" style="width:${Math.max(0,Math.min(100,ev.score))}%"></div></div>`;
    btn.onclick=()=>{activeRound=r;render();};
...
function renderRounds(){
  ...
  el.className='round' + (ev.checks.some(c=>c[1]==='bad') ? ' invalid' : '');
  el.innerHTML=`
    <div class="round-head">
      <div><p class="eyebrow">Focus mode</p><h3>Round ${r+1}</h3></div>
      <span class="score">${ev.score}/100</span>
    </div>
    <div class="slots"></div>
    <div class="checks">${ev.checks.map(([t,k])=>`<span class="check ${k}">${t}</span>`).join('')}</div>
    <div class="round-actions"><button class="small" data-opt="${r}">Optimize Round</button><button class="small" data-clear="${r}">Clear Round</button><button class="small" id="nextRoundBtn">Next Round →</button></div>
    <div class="suggest">${suggestions(r)}</div>`;
  const slots=el.querySelector('.slots');
  for(let s=0;s<6;s++){
    const sl=document.createElement('div');
    sl.className='slot'+(lockedSlots[r][s]?' locked':'');
    sl.dataset.r=r; sl.dataset.s=s;
    const p=getP(plan[r][s]);
    if(p){
      const c=card(p); c.draggable=true;
      c.addEventListener('dblclick',()=>{plan[r][s]=null;save();render();});
      sl.appendChild(c);
    } else {
      sl.innerHTML='<div class="slot-empty">Drop player here</div>';
    }
    sl.addEventListener('dragover',e=>e.preventDefault());
    sl.addEventListener('drop',()=>{if(!dragId)return; place(dragId,r,s);});
    sl.addEventListener('contextmenu',e=>{e.preventDefault();lockedSlots[r][s]=!lockedSlots[r][s];save();render();});
    slots.appendChild(sl);
  }
  root.innerHTML=''; root.appendChild(el);
  const opt=root.querySelector('[data-opt]'); if(opt) opt.onclick=()=>{for(let i=0;i<5;i++) forceFixRound(r); improveRound(r); for(let i=0;i<5;i++) forceFixRound(r); save();render();};
  const clr=root.querySelector('[data-clear]'); if(clr) clr.onclick=()=>{plan[r]=Array(6).fill(null);save();render();};
  const next=root.querySelector('#nextRoundBtn'); if(next) next.onclick=()=>{activeRound=(activeRound+1)%7;render();};
}

function renderAssistant(){
  const el=document.getElementById('assistantPanel');
  if(!el) return;
  const ev=evaluate(activeRound);
  const bad=ev.checks.filter(c=>c[1]==='bad');
  const warn=ev.checks.filter(c=>c[1]==='warn');
  const good=ev.checks.filter(c=>!c[1]);
  const allBad=[];
  for(let r=0;r<7;r++){
    const e=evaluate(r);
    const missing=e.checks.filter(c=>c[1]==='bad').map(c=>c[0]);
    if(missing.length) allBad.push({r,missing});
  }
  const bestAvail = players.filter(p=>!inAnyPlan(p.id) && canPlace(p.id,activeRound)).slice(0,6);
  el.innerHTML=`
    <h2>Round ${activeRound+1} Assistant</h2>
    <div class="assistant-block">
      <h4>Validation</h4>
      <div class="assistant-list">
        ${[...bad,...warn,...good].map(([t,k])=>`<span class="check ${k}">${t}</span>`).join('')}
      </div>
    </div>
    <div class="assistant-block">
      <h4>Best Available</h4>
      <div class="assistant-list">
        ${bestAvail.length?bestAvail.map(p=>`<div class="assistant-item"><b>${p.ign}</b><span>${info(p).short} · ${p.owner}</span></div>`).join(''):'<div class="assistant-item"><span>No clean available pick right now.</span></div>'}
      </div>
    </div>
    <div class="assistant-block">
      <h4>All Critical Rounds</h4>
      <div class="assistant-list">
        ${allBad.length?allBad.map(x=>`<div class="assistant-item"><b>R${x.r+1}</b><span>${x.missing.join(', ')}</span></div>`).join(''):'<div class="assistant-item"><b>All clear</b><span>No hard-rule issues.</span></div>'}
      </div>
    </div>
    <div class="assistant-block">
      <h4>Tip</h4>
      <div class="assistant-item"><span>Right-click a slot to lock it. Double-click a filled player to remove from the round.</span></div>
    </div>`;
}

function renderPool(){
  const root=document.getElementById('playerPool'); if(!root) return;
  const search=document.getElementById('searchInput');
  const q=(search?.value||'').toLowerCase();
  const list=players.filter(p=>(p.ign+p.cls+p.owner).toLowerCase().includes(q));
  const count=document.getElementById('poolCount'); if(count) count.textContent=`${list.length} shown`;
  root.innerHTML='';
  list.forEach(p=>{const c=card(p); if(inAnyPlan(p.id)) c.classList.add('used'); root.appendChild(c);});
}

(function setupV10Drawer(){
  const drawer=document.getElementById('drawer'), shade=document.getElementById('drawerShade');
  const open=()=>{drawer?.classList.add('open'); shade?.classList.add('open');};
  const close=()=>{drawer?.classList.remove('open'); shade?.classList.remove('open');};
  const openBtn=document.getElementById('openDrawerBtn'), closeBtn=document.getElementById('closeDrawerBtn');
  if(openBtn) openBtn.onclick=open;
  if(closeBtn) closeBtn.onclick=close;
  if(shade) shade.onclick=close;
})();
