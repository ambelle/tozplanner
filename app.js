...
const STORAGE_KEY = 'tozAutoPlanner_v3_ownerRules';
...
const samplePublics = [["PublicMM","Marksman",240,"PublicMM",1,7],["PublicNL","Night Lord",245,"PublicNL",1,3],["PublicDK","Dark Knight",230,"PublicDK",3,7],["PublicBW","Blaze Wizard",235,"PublicBW",4,7],["PublicSair","Corsair",240,"PublicSair",1,7],["PublicShad","Shadower",240,"PublicShad",2,5]];
...
document.getElementById('addPlayerBtn').onclick=()=>addPlayer(ignInput.value.trim()||`Player${players.length+1}`,classInput.value,levelInput.value,ownerInput.value||ignInput.value.trim()||'Public',fromRound.value,toRound.value);
...
document.getElementById('samplePublicBtn').onclick=()=>samplePublics.forEach(x=>{if(!players.some(p=>p.ign===x[0]))addPlayer(...x)});
